...
def main():
    ...
    try:
        ...
        while True:
            ...
            # Define green to yellow range in HSV
            # Hue: Green ~60, Yellow ~30
            lower_green = np.array([30, 40, 40])
            upper_yellow = np.array([80, 255, 255])
            
            # Create mask for green-yellow colors
            mask = cv2.inRange(hsv, lower_green, upper_yellow)
            
            # Apply mask to original frame
            masked_frame = cv2.bitwise_and(frame, frame, mask=mask)
            
            # Convert masked BGR to RGB for prediction
            rgb_frame = cv2.cvtColor(masked_frame, cv2.COLOR_BGR2RGB)
            
            # Make prediction only if enough green-yellow pixels
            if np.count_nonzero(mask) > 1000:  # Adjust threshold as needed
            ...
            # Show original and masked frames
            cv2.imshow('Original', frame)
            cv2.imshow('Masked Plant', masked_frame)
